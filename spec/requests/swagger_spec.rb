...
RSpec.describe 'Swagger spec', type: :apivore, order: :defined do
  ...
  context 'GET /auctions' do
    context 'when it throws an exception' do
      it 'should return a ??? error' do
        skip('Clarify error conditions')
        allow_any_instance_of(AuctionQuery).to receive(:public_index).and_raise(StandardError)
